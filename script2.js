document.addEventListener('DOMContentLoaded', () => {
    ...
    // Función para generar los campos de materias para cada cuatrimestre
    window.generateTerms = function() {
        const selectedCareer = careerOptions.value;
        const numTerms = parseInt(numTermsInput.value) || 0;

        termsContainer.innerHTML = ''; // Limpiar cuatrimestres anteriores
        resultContainer.innerHTML = ''; // Limpiar resultados anteriores

        if (!selectedCareer) {
            resultContainer.innerHTML = 'Debes seleccionar una carrera.';
            return;
        }

        if (numTerms <= 0) {
            resultContainer.innerHTML = 'El número de cuatrimestres debe ser mayor que 0.';
            return;
        }

        for (let i = 0; i < numTerms; i++) {
            const termDiv = document.createElement('div');
            termDiv.classList.add('term-container');
            termDiv.innerHTML = `
                <h3>Cuatrimestre ${i + 1}</h3>
                <div class="form-group">
                    <label for="numSubjects-${i}">Número de materias:</label>
                    <input type="number" id="numSubjects-${i}" class="form-control" min="1" value="1">
                </div>
                <div id="subjects-${i}"></div>
            `;
            termsContainer.appendChild(termDiv);

            // Agregar materias a este cuatrimestre
            const numSubjectsInput = document.getElementById(`numSubjects-${i}`);
            numSubjectsInput.addEventListener('input', () => updateSubjects(i));
            updateSubjects(i);
        }
    };

    // Función para actualizar las materias disponibles en cada cuatrimestre
    function updateSubjects(termIndex) {
        const numSubjects = parseInt(document.getElementById(`numSubjects-${termIndex}`).value) || 0;
        const subjectsContainer = document.getElementById(`subjects-${termIndex}`);
        const selectedCareer = careerOptions.value;
        const subjects = subjectsByCareer[selectedCareer] || {};

        subjectsContainer.innerHTML = ''; // Limpiar materias anteriores

        if (selectedCareer && numSubjects > 0) {
            for (let i = 0; i < numSubjects; i++) {
                const subjectDiv = document.createElement('div');
                subjectDiv.classList.add('subject-selection');
                subjectDiv.innerHTML = `
                    <div class="form-group">
                        <label for="subject-${termIndex}-${i}">Materia:</label>
                        <select id="subject-${termIndex}-${i}" class="form-control">
                    ...
                    <div class="form-group">
                        <label for="grade-${termIndex}-${i}">Calificación:</label>
                        <select id="grade-${termIndex}-${i}" class="form-control">
    ...
    window.calculateGlobalIndex = function() {
        const selectedCareer = careerOptions.value;
        if (!selectedCareer) {
            resultContainer.innerHTML = 'Debes seleccionar una carrera.';
            return;
        ...
        for (let i = 0; i < numTerms; i++) {
            const numSubjects = parseInt(document.getElementById(`numSubjects-${i}`).value) || 0;
            if (numSubjects <= 0) {
                resultContainer.innerHTML = `El número de materias para el cuatrimestre ${i + 1} debe ser mayor que 0.`;
                return;
            }

            for (let j = 0; j < numSubjects; j++) {
                const subjectSelect = document.getElementById(`subject-${i}-${j}`);
                const gradeSelect = document.getElementById(`grade-${i}-${j}`);
                ...
                if (!subjectName || !grade) {
                    resultContainer.innerHTML = `Debes seleccionar materia y calificación para cada materia en el cuatrimestre ${i + 1}.`;
                    return;
                }

                const subjectCredits = subjectsByCareer[selectedCareer][subjectName];
                let gradePoints;
        ...
        resultContainer.innerHTML = `Índice Global: ${index.toFixed(2)}`;
    };
});
